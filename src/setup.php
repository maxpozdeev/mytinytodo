...
if ($ver == '')
{
    ...
    elseif ($install == 'create')
    {
        ...
        # create default list
        $db->ex( "INSERT INTO {$db->getPrefix()}lists (uuid,name,d_created,taskview) VALUES (?,?,?,?)", array(generateUUID(), 'Todo', time(), 1) );
...
function createMysqlTables(Database_Abstract $db)
{
    ...
    $db->ex(
"CREATE TABLE {$db->getPrefix()}lists (
    ...
    $db->ex(
"CREATE TABLE {$db->getPrefix()}todolist (
    `id` INT UNSIGNED NOT NULL auto_increment,
    `uuid` CHAR(36) CHARACTER SET latin1 NOT NULL default '',
    ...
    $db->ex(
"CREATE TABLE {$db->getPrefix()}tags (
    `id` INT UNSIGNED NOT NULL auto_increment,
    `name` VARCHAR(250) NOT NULL default '',
    PRIMARY KEY(`id`),
    ...
    $db->ex(
"CREATE TABLE {$db->getPrefix()}tag2task (
    ...
    $db->ex(
"CREATE TABLE {$db->getPrefix()}settings (
    ...
    $db->ex(
"CREATE TABLE {$db->getPrefix()}sessions (
...
"CREATE TABLE {$db->getPrefix()}lists (
    ...
    $db->ex("CREATE UNIQUE INDEX {$db->getPrefix()}lists_uuid ON {$db->getPrefix()}lists (uuid)");
...
"CREATE TABLE {$db->getPrefix()}todolist (
    ...
    $db->ex("CREATE INDEX {$db->getPrefix()}todo_list_id ON {$db->getPrefix()}todolist (list_id)");
    $db->ex("CREATE UNIQUE INDEX {$db->getPrefix()}todo_uuid ON {$db->getPrefix()}todolist (uuid)");
...
"CREATE TABLE {$db->getPrefix()}tags (
    ...
    $db->ex("CREATE UNIQUE INDEX {$db->getPrefix()}tags_lower_name ON {$db->getPrefix()}tags ((LOWER(name)))");
...
"CREATE TABLE {$db->getPrefix()}tag2task (
    ...
    $db->ex("CREATE INDEX {$db->getPrefix()}tag2task_tag_id ON {$db->getPrefix()}tag2task (tag_id)");
    $db->ex("CREATE INDEX {$db->getPrefix()}tag2task_task_id ON {$db->getPrefix()}tag2task (task_id)");
    $db->ex("CREATE INDEX {$db->getPrefix()}tag2task_list_id ON {$db->getPrefix()}tag2task (list_id)");
...
"CREATE TABLE {$db->getPrefix()}settings (
    ...
    $db->ex("CREATE UNIQUE INDEX {$db->getPrefix()}settings_key ON {$db->getPrefix()}settings (param_key)");
...
"CREATE TABLE {$db->getPrefix()}sessions (
    ...
    $db->ex("CREATE UNIQUE INDEX {$db->getPrefix()}sessions_id ON {$db->getPrefix()}sessions (id)");
}
...
"CREATE TABLE {$db->getPrefix()}lists (
    ...
    $db->ex("CREATE UNIQUE INDEX lists_uuid ON {$db->getPrefix()}lists (uuid)");
...
"CREATE TABLE {$db->getPrefix()}todolist (
    ...
    $db->ex("CREATE INDEX todo_list_id ON {$db->getPrefix()}todolist (list_id)");
    $db->ex("CREATE UNIQUE INDEX todo_uuid ON {$db->getPrefix()}todolist (uuid)");
...
"CREATE TABLE {$db->getPrefix()}tags (
    ...
    $db->ex("CREATE INDEX tags_name ON {$db->getPrefix()}tags (name)"); //NB: unique in mysql
...
"CREATE TABLE {$db->getPrefix()}tag2task (
    ...
    $db->ex("CREATE INDEX tag2task_tag_id ON {$db->getPrefix()}tag2task (tag_id)");
    $db->ex("CREATE INDEX tag2task_task_id ON {$db->getPrefix()}tag2task (task_id)");
    $db->ex("CREATE INDEX tag2task_list_id ON {$db->getPrefix()}tag2task (list_id)");    /* for tagcloud */
...
"CREATE TABLE {$db->getPrefix()}settings (
    ...
    $db->ex("CREATE UNIQUE INDEX settings_key ON {$db->getPrefix()}settings (param_key COLLATE NOCASE)");
...
"CREATE TABLE {$db->getPrefix()}sessions (
    ...
    $db->ex("CREATE UNIQUE INDEX sessions_id ON {$db->getPrefix()}sessions (id COLLATE NOCASE)");
}
...
{
    if ( !$db ) return '';
    if ( !$db->tableExists($db->getTableWithPrefix('todolist')) ) return '';
    $v = '1.0';
    if ( !$db->tableExists($db->getTableWithPrefix('tags')) ) return $v;
    $v = '1.1';
    if ( !$db->tableFieldExists($db->getTableWithPrefix('todolist'), 'duedate') ) return $v;
    $v = '1.2';
    if ( !$db->tableExists($db->getTableWithPrefix('lists')) ) return $v;
    $v = '1.3.0';
    if ( !$db->tableFieldExists($db->getTableWithPrefix('todolist'), 'd_completed') ) return $v;
    $v = '1.3.1';
    if ( !$db->tableFieldExists($db->getTableWithPrefix('todolist'), 'd_edited') ) return $v;
    $v = '1.4';
    if ( !$db->tableExists($db->getTableWithPrefix('settings')) ) return $v;
    $v = '1.7';
    if ( $db->tableFieldExists($db->getTableWithPrefix('todolist'), 'tags') ) return $v;
    $v = '1.8';
    return $v;
}
...
function update_14_17(Database_Abstract $db, $dbtype)
{
    ...
    if($dbtype=='mysql')
    {
        $db->ex("ALTER TABLE {$db->getPrefix()}lists ADD `extra` TEXT");

        # increase the length of list and tag name
        # (not applicable to sqlite because it uses VARCHAR fields of any length as TEXT)
        $db->ex("ALTER TABLE {$db->getPrefix()}todolist CHANGE `tags` `tags` VARCHAR(2000) NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}tags CHANGE `name` `name` VARCHAR(250) NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}lists CHANGE `name` `name` VARCHAR(250) NOT NULL default '' ");

        # convert charset to utf8mb4

        $db->ex("ALTER TABLE {$db->getPrefix()}lists    CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        $db->ex("ALTER TABLE {$db->getPrefix()}todolist CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        $db->ex("ALTER TABLE {$db->getPrefix()}tags     CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
        $db->ex("ALTER TABLE {$db->getPrefix()}tag2task CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci");
...
function update_17_18(Database_Abstract $db, $dbtype)
{
    ...
    if ($dbtype == 'sqlite')
    {
        ...
        $db->ex("ALTER TABLE {$db->getPrefix()}todolist RENAME TO {$db->getPrefix()}todolist_old");
        $db->ex(
            "CREATE TABLE {$db->getPrefix()}todolist (
                id INTEGER PRIMARY KEY,
                uuid CHAR(36) NOT NULL default '',
        ...
        $db->ex("INSERT INTO {$db->getPrefix()}todolist SELECT id,uuid,list_id,d_created,d_completed,d_edited,compl,title,note,prio,ow,duedate FROM {$db->getPrefix()}todolist_old");
        $db->ex("CREATE INDEX todo_list_id ON {$db->getPrefix()}todolist (list_id)");
        $db->ex("CREATE UNIQUE INDEX todo_uuid ON {$db->getPrefix()}todolist (uuid)");
        $db->ex("DROP TABLE {$db->getPrefix()}todolist_old");

        $db->ex("DROP INDEX tags_name");
        $db->ex("ALTER TABLE {$db->getPrefix()}tags RENAME TO {$db->getPrefix()}tags_old");
        $db->ex(
            "CREATE TABLE {$db->getPrefix()}tags (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                name VARCHAR(250) NOT NULL DEFAULT '' COLLATE UTF8CI )"
        );
        $db->ex("INSERT INTO {$db->getPrefix()}tags SELECT * FROM {$db->getPrefix()}tags_old");
        $db->ex("CREATE INDEX tags_name ON {$db->getPrefix()}tags (name)");
        $db->ex("DROP TABLE {$db->getPrefix()}tags_old");
    }
    else // mysql
    {
        $db->ex("ALTER TABLE {$db->getPrefix()}todolist DROP COLUMN tags");
        $db->ex("ALTER TABLE {$db->getPrefix()}todolist DROP COLUMN tags_ids");
        ...
        $db->ex("ALTER TABLE {$db->getPrefix()}lists MODIFY `uuid` CHAR(36) CHARACTER SET latin1 NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}lists MODIFY `name` VARCHAR(250) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}lists MODIFY `extra` TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci ");

        $db->ex("ALTER TABLE {$db->getPrefix()}todolist MODIFY `uuid` CHAR(36) CHARACTER SET latin1 NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}todolist MODIFY `title` VARCHAR(250) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}todolist MODIFY `note` TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci ");

        $db->ex("ALTER TABLE {$db->getPrefix()}tags MODIFY `name` VARCHAR(250) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci NOT NULL default '' ");

        $db->ex("ALTER TABLE {$db->getPrefix()}settings MODIFY `param_key` VARCHAR(250) CHARACTER SET latin1 NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}settings MODIFY `param_value` TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci ");

        $db->ex("ALTER TABLE {$db->getPrefix()}sessions MODIFY `id` VARCHAR(64) CHARACTER SET latin1 NOT NULL default '' ");
        $db->ex("ALTER TABLE {$db->getPrefix()}sessions MODIFY `data` TEXT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_520_ci ");
    }
